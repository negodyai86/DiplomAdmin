...
resource "yandex_compute_instance" "zabbix" {
  ...
  network_interface {
    ...
    security_group_ids = [yandex_vpc_security_group.aleks-public.id]
  }
  metadata = {
    user-data = "${file("./meta.txt")}"
  }
...
resource "yandex_compute_instance" "bastion-host" {
  ...
  network_interface {
    ...
    nat                = true
    security_group_ids = [yandex_vpc_security_group.aleks-gw.id]
...
resource "yandex_alb_load_balancer" "alb1" {
  name               = "alb1"
  network_id         = yandex_vpc_network.network1.id
  security_group_ids = [yandex_vpc_security_group.aleks-balancer.id]
...
resource "yandex_vpc_security_group" "aleks-balancer" {
  name       = "aleks-balancer"
  network_id = yandex_vpc_network.network1.id
...
resource "yandex_vpc_security_group" "aleks-private" {
  name       = "aleks-private"
  network_id = yandex_vpc_network.network1.id
  ...
  ingress {
    protocol          = "ANY"
    description       = "any"
    security_group_id = yandex_vpc_security_group.aleks-gw.id
  }
...
resource "yandex_vpc_security_group" "aleks-public" {
  name       = "aleks-public"
  network_id = yandex_vpc_network.network1.id
...
resource "yandex_vpc_security_group" "aleks-gw" {
  name       = "aleks-gw"
  network_id = yandex_vpc_network.network1.id
